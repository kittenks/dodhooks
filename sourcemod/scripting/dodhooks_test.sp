...
void TestNative(const char[] name)
{
    ...
    const char[] statusStr =
        status == FeatureStatus_Available ? "OK" :
        status == FeatureStatus_Unknown ? "UNKNOWN" :
        status == FeatureStatus_NotAvailable ? "NOT AVAILABLE" : "ERROR";
    PrintToServer("[DODHOOKS-TEST]   %-32s %s", name, statusStr);
}
